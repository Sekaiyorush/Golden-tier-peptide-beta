    ...
    <section className="py-24 md:py-32 relative overflow-hidden bg-white">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {isLoading ? (
            ...
            featuredProducts.map((product) => (
              ...
              >
                {/* Product Image Area */}
                <Link to={`/product/${product.sku}`} className="relative block aspect-[4/3] bg-slate-50 overflow-hidden p-8 flex items-center justify-center border-b border-[#D4AF37]/10">
                  ...
                  <div className="flex items-end justify-between pt-6 mt-auto border-t border-[#D4AF37]/10">
                    ...
                    {isPartner && (
                      <button
                        ...
                        className="relative flex items-center justify-center w-12 h-12 bg-[#111] text-white hover:bg-black transition-all disabled:opacity-50 border border-[#111] group-btn shadow-md overflow-hidden"
                      >
        ...
        <div className="flex justify-center mt-20">
          <Link
            to="/products"
            className="group flex items-center justify-center space-x-3 px-10 py-5 border border-[#D4AF37]/30 text-slate-900 transition-all duration-500 hover:border-[#D4AF37] hover:shadow-[0_10px_30px_rgba(212,175,55,0.1)] bg-white relative overflow-hidden"
          >
            <span className="relative z-10 text-[10px] font-bold tracking-[0.3em] uppercase">VIEW FULL CATALOG</span>
            <ArrowRight className="relative z-10 h-4 w-4 text-[#D4AF37] group-hover:translate-x-2 transition-transform duration-300" />
            <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-gradient-to-r from-[#D4AF37] to-[#F3E5AB] transition-all duration-500 ease-out group-hover:w-full" />
          </Link>
        </div>
      </div>
